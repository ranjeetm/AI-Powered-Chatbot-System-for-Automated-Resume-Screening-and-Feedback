...
def home_layout():
    ...
    with stat_col1:
        render_stat_card("Total Resumes", stats["total"], "📄", "#667eea")
    
    with stat_col2:
        avg_display = f"{stats['avg_score']:.1f}%" if stats['avg_score'] > 0 else "N/A"
        render_stat_card("Average Score", avg_display, "📈", "#f093fb")
    
    with stat_col3:
        top_display = f"{stats['top_score']:.1f}%" if stats['top_score'] > 0 else "N/A"
        render_stat_card("Top Score", top_display, "🏆", "#4ade80")
    
    with stat_col4:
        render_stat_card("Qualified", stats["qualified"], "✅", "#fb923c")
